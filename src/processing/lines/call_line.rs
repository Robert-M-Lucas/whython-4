...
impl LineHandler for CallLine {
    fn process_line(line: &Vec<Symbol>, memory_managers: &mut MemoryManagers,
        ...
        if line.len() != 2 { return ProcessingResult::Unmatched; }

        if !matches!(line[0], Symbol::Name(_)) { return ProcessingResult::Unmatched; }
        if !matches!(line[1], Symbol::ArithmeticBlock(_)) { return ProcessingResult::Unmatched; }

        match handle_arithmetic_section(memory_managers,
