...
pub const OR_INSTRUCTION_CODE: u16 = 8;

/// Applies OR to LHS and RHS
impl OrInstruction {
    pub fn new_alloc(memory_managers: &mut MemoryManagers, lhs: usize, rhs: usize, dest: usize) -> Self {
        let mut instruction_memory = vec![];
        instruction_memory.extend(OR_INSTRUCTION_CODE.to_le_bytes());
    ...
    pub fn get_code() -> u16 { OR_INSTRUCTION_CODE }
    ...
    pub fn execute(pointer: &mut usize, memory_managers: &mut MemoryManagers) {
        let lhs = memory_managers.variable_memory.memory[get_usize(pointer, &memory_managers.program_memory.memory)];
        *pointer += size_of::<usize>();
        let rhs = memory_managers.variable_memory.memory[get_usize(pointer, &memory_managers.program_memory.memory)];
        *pointer += size_of::<usize>();
